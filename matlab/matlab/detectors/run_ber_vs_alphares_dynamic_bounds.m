function out = run_ber_vs_alphares_dynamic_bounds(user_cfg)
%RUN_BER_VS_ALPHARES_DYNAMIC_BOUNDS
% Sequence-average BER vs alpha_max_res under dynamic/static C1 policies.
%
% Curves:
% 1) dynamic_oracle
% 2) static_max
...
fprintf('==== BER vs alpha_max_res: dynamic bounds experiment ====\n');
...
seq_level_alpha_eff_stats = repmat(struct( ...
    ...
    'seq_alpha_eff_min', [], ...
    'seq_alpha_eff_mean', [], ...
    'seq_alpha_eff_std', []), num_alpha, 1);
seq_level_best_c1_stats = repmat(struct( ...
    'alpha_max_res_cfg', 0.0, ...
    'alpha_max_raw_cfg', 0.0, ...
    'best_idx_frame', [], ...
    'best_c1_frame', [], ...
    'seq_best_idx_mean', [], ...
    'seq_best_idx_std', [], ...
    'seq_best_idx_switch_rate', [], ...
    'seq_best_c1_mean', [], ...
    'seq_best_c1_std', [], ...
    'seq_ber_dynamic_oracle', [], ...
    'seq_ber_static_max', [], ...
    'seq_ber_static_min', [], ...
    'seq_ber_static_mean', []), num_alpha, 1);
...
for ai = 1:num_alpha
    ...
    seq_best_idx_mean = zeros(cfg_alpha.num_seq, 1);
    seq_best_idx_std = zeros(cfg_alpha.num_seq, 1);
    seq_best_idx_switch_rate = zeros(cfg_alpha.num_seq, 1);
    seq_best_c1_mean = zeros(cfg_alpha.num_seq, 1);
    seq_best_c1_std = zeros(cfg_alpha.num_seq, 1);
    ...
    for sid = 1:cfg_alpha.num_seq
        ...
        frame_alpha_eff(sid, :) = alpha_eff_vec(:).';

        seq_alpha_max(sid) = max(alpha_eff_vec);
        seq_alpha_min(sid) = min(alpha_eff_vec);
        seq_alpha_mean(sid) = mean(alpha_eff_vec);
        seq_alpha_std(sid) = std(alpha_eff_vec);

        c1_static_max = compute_c1_base_from_alpha(seq_alpha_max(sid), sys);
        c1_static_min = compute_c1_base_from_alpha(seq_alpha_min(sid), sys);
        c1_static_mean = compute_c1_base_from_alpha(seq_alpha_mean(sid), sys);
        ...
        for tt = 1:cfg_alpha.num_frames
            frame = seq_data.frames(tt);

            sweep = run_frame_c1_sweep(frame, cfg_alpha, sys);
            best_idx_frame(sid, tt) = sweep.best_idx;
            best_c1_frame(sid, tt) = sweep.best_c1;

            err_dynamic = err_dynamic + min(sweep.bit_err_grid);
            bits_dynamic = bits_dynamic + sweep.bits_per_eval;

            [err_now, bits_now] = evaluate_frame_for_c1(frame, c1_static_max, cfg_alpha, sys);
            err_static_max = err_static_max + err_now;
            bits_static_max = bits_static_max + bits_now;

            [err_now, bits_now] = evaluate_frame_for_c1(frame, c1_static_min, cfg_alpha, sys);
            err_static_min = err_static_min + err_now;
            bits_static_min = bits_static_min + bits_now;

            [err_now, bits_now] = evaluate_frame_for_c1(frame, c1_static_mean, cfg_alpha, sys);
            err_static_mean = err_static_mean + err_now;
            bits_static_mean = bits_static_mean + bits_now;
        end

        seq_ber_dynamic(sid) = err_dynamic / max(bits_dynamic, 1);
        seq_ber_static_max(sid) = err_static_max / max(bits_static_max, 1);
        seq_ber_static_min(sid) = err_static_min / max(bits_static_min, 1);
        seq_ber_static_mean(sid) = err_static_mean / max(bits_static_mean, 1);

        seq_best_idx_mean(sid) = mean(best_idx_frame(sid, :));
        seq_best_idx_std(sid) = std(best_idx_frame(sid, :));
        if cfg_alpha.num_frames > 1
            seq_best_idx_switch_rate(sid) = mean(diff(best_idx_frame(sid, :)) ~= 0);
        else
            seq_best_idx_switch_rate(sid) = 0.0;
        end
        seq_best_c1_mean(sid) = mean(best_c1_frame(sid, :));
        seq_best_c1_std(sid) = std(best_c1_frame(sid, :));

        if mod(sid, cfg_alpha.progress_every_seq) == 0 || sid == cfg_alpha.num_seq
            fprintf('  progress %4d/%4d | dyn=%.3e stat_mean=%.3e\n', ...
                sid, cfg_alpha.num_seq, mean(seq_ber_dynamic(1:sid)), mean(seq_ber_static_mean(1:sid)));
        end
    end
    ...
    seq_level_alpha_eff_stats(ai) = struct( ...
    ...
    seq_level_best_c1_stats(ai) = struct( ...
        'alpha_max_res_cfg', cfg_alpha.alpha_max_res, ...
        'alpha_max_raw_cfg', cfg_alpha.alpha_max_raw, ...
        'best_idx_frame', best_idx_frame, ...
        'best_c1_frame', best_c1_frame, ...
        'seq_best_idx_mean', seq_best_idx_mean, ...
        'seq_best_idx_std', seq_best_idx_std, ...
        'seq_best_idx_switch_rate', seq_best_idx_switch_rate, ...
        'seq_best_c1_mean', seq_best_c1_mean, ...
        'seq_best_c1_std', seq_best_c1_std, ...
    ...
    fprintf('  alpha_res=%.2e | dyn=%.3e | static_max=%.3e | static_min=%.3e | static_mean=%.3e\n', ...
        cfg_alpha.alpha_max_res, ber_dynamic_oracle(ai), ber_static_max(ai), ber_static_min(ai), ber_static_mean(ai));
end

fig = figure('Color', 'w');
semilogy(cfg.alpha_res_list, ber_dynamic_oracle, '-o', 'LineWidth', 1.6, 'DisplayName', 'dynamic_oracle');
hold on;
semilogy(cfg.alpha_res_list, ber_static_max, '-s', 'LineWidth', 1.6, 'DisplayName', 'static_max');
semilogy(cfg.alpha_res_list, ber_static_min, '-d', 'LineWidth', 1.6, 'DisplayName', 'static_min');
semilogy(cfg.alpha_res_list, ber_static_mean, '-^', 'LineWidth', 1.6, 'DisplayName', 'static_mean');
grid on;
xlabel('\alpha_{max,res}^{cfg}');
ylabel('Sequence-average BER');
title('BER vs \alpha_{max,res}: dynamic oracle vs static bounds');
legend('Location', 'best');
save_figure_pair(fig, fullfile(cfg.output_dir, 'fig_ber_vs_alphares_dynamic_bounds'));
...
results.seq_level_alpha_eff_stats = seq_level_alpha_eff_stats;
results.seq_level_best_c1_stats = seq_level_best_c1_stats;
results.meta_cfg = cfg;

save(cfg.output_mat, 'results', '-v7.3');

fprintf('\nSummary table\n');
fprintf(' alpha_res       alpha_raw       dynamic       static_max    static_min    static_mean   dyn-mean      dyn/mean\n');
for ai = 1:num_alpha
    diff_dyn_mean = ber_dynamic_oracle(ai) - ber_static_mean(ai);
    ratio_dyn_mean = ber_dynamic_oracle(ai) / max(ber_static_mean(ai), 1e-12);
    fprintf(' %10.2e   %10.2e   %10.3e   %10.3e   %10.3e   %10.3e   %10.3e   %8.4f\n', ...
        cfg.alpha_res_list(ai), cfg.alpha_raw_list(ai), ...
        ber_dynamic_oracle(ai), ber_static_max(ai), ber_static_min(ai), ber_static_mean(ai), ...
        diff_dyn_mean, ratio_dyn_mean);
end

fprintf('\nSaved figure: %s\n', fullfile(cfg.output_dir, 'fig_ber_vs_alphares_dynamic_bounds.png'));
...
cfg.output_mat = fullfile(cfg.output_dir, 'results_ber_vs_alphares_dynamic_bounds.mat');
