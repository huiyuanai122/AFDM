function [x_hat, diagnostics] = oampnet_detector(y, H, noise_var, params, Q)
% =========================================================================
...
% =========================================================================
    ...
    for t = 1:num_layers
        ...
        if need_diag && t == num_layers
            [x_nle, logits_last, prob_last] = qpsk_mmse_estimator(r, tau_vec * temp_t);
            tau_vec_last = tau_vec;
            gamma_last = gamma_t;
            temperature_last = temp_t;
        else
    ...
    x_hat = x;
    if need_diag
        diagnostics.logits_last = logits_last;
        diagnostics.prob_last = prob_last;
        diagnostics.tau_vec_last = tau_vec_last;
        diagnostics.gamma_last = gamma_last;
        diagnostics.damping_last = damping_last;
        diagnostics.temperature_last = temperature_last;
    end
end


function [x_mmse, logits, prob] = qpsk_mmse_estimator(r, tau)
% QPSK MMSE估计器
% 输入:
%   r   - 线性估计器输出 [N × 1]
