...
% New unified time-vary state helpers.
...
use_sequence_channel_state = true; % New logic toggle (default on).
...
tv_cfg = get_timevary_defaults(struct( ...
    'rho_alpha', 0.98, ...
    'rho_h', 0.98, ...
    'alpha_max', alpha_max, ...
...
for mc = 1:num_mc
    % Legacy behavior:
    %   one static channel over the full stream.
    % New unified behavior:
    %   channel state is generated by common sequence-level state machine.
    if use_sequence_channel_state
        [tv_state, ch] = init_timevary_channel_state(P, ell_max, alpha_max, tv_cfg);
        % TODO: Stream model below still assumes one channel for the whole stream.
        % We map to target-block channel by stepping to targetBlock.
        for bb = 2:targetBlock
            [tv_state, ch] = step_timevary_channel_state(tv_state); %#ok<NASGU>
        end
    else
...
% Legacy shim kept for compatibility with this demo file.
[~, ch] = init_timevary_channel_state(P, ell_max, alpha_max, struct( ...
    'rho_alpha', 0.0, ...
    'rho_h', 0.0, ...
    'ell_mode', 'static', ...
    'pdp_mode', 'exp_fixed_per_sequence'));
